...
describe("Crowdsale",async()=>{
    ...
    describe("Buying Tokens",async()=>{
        ...
        describe("Success",async()=>{
            beforeEach(async()=>{
                transaction = await crowdsale.connect(user1).reserveTokens(tokens(10),{value : ether(10)})
        ...
        describe("Failure",async()=>{
            it("reject insufficient ETH",async()=>{         
                await expect(crowdsale.connect(user1).reserveTokens(tokens(10),{value:0})).to.be.reverted
            })

            it("reject buy transaction if the opentime > block.timestamp",async()=>{  
                await time.increaseTo(OPENINGTIME+1005)       
                await expect(crowdsale.connect(user1).reserveTokens(tokens(10),{value:10})).to.be.reverted
            })
